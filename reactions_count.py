...
def get_reactions_count(ids: List[int]) -> Dict[int, int]:
    """
    Получает количество реакций (лайков) для заданных идентификаторов тредов и комментариев.

    Аргументы:
        - `ids (List[int])`: Список идентификаторов тредов и комментариев.

    Возвращает:
        `Dict[int, int]`: Словарь, в котором ключ - это идентификатор треда или комментария, а значение - количество лайков.
    """
